...
import {
  errorMissingDataAlert,
  interactiveAlert,
  confirmSendDataAlert,
  loadingAlert,
  noSelectedAlert,
  errorNoOfferAlert,
  errorGettingOffers,
...
async function validateFields(recordData, requiredField) {
  try {
    if (!recordData)
      return interactiveAlert("Error: On validate data", "error").then(() =>
        closeWidget()
      );
...
async function displayOffers() {
  if (!crmUser.ware.id)
    return noSelectedAlert("Tienes que seleccionar un Cluster");

  loadingAlert("Buscando Beneficios...", 4000);
  let offersAPI = await getOffers(crmUser.ware.id);
  let message = offersAPI.data.message;

  if (offersAPI.code === 404)
    return errorNoOfferAlert(`${crmUser.ware.name}`, message);

  if (offersAPI.code !== 404 && offersAPI.code !== 200)
    return errorGettingOffers(message);
...
function sendOffer() {
  if (!crmUser.offer.id)
    return noSelectedAlert("Tienes que seleccionar una Beneficio");
      ...
      sendMailCRM(crmUser).then((response) => {
        if (!response) {
          return interactiveAlert(
            "Hubo un error en el envío de Información.\nFavor ver notas.",
            "error"
          ).then((closeOk) => {
            if (closeOk) {
              closeWidgetReload();
...
async function validateStages(recordData, validStages) {
  try {
    if (!recordData)
      return interactiveAlert("Error: Validate Stages", "error").then(() =>
        closeWidget()
      );
    const formStage = recordData["Stage"];

    let message =
      'No es posible crear el botón de pago. Por favor cambia el stage a "Próximo a cerrar" e intenta nuevamente.';
    if (!validStages.includes(formStage))
      return interactiveAlert(message, "info").then(() => closeWidget());
    return true;
  } catch (error) {
